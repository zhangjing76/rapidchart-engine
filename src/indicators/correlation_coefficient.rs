...
mod tests {
    ...
    #[test]
    fn correlation_coefficient_is_one_for_a_perfect_line() {
        let store = close_store(&[1.0, 2.0, 3.0]);
        let values = correlation_coefficient_store(&store, 3, &mut HashMap::new());

        assert_series_close(&values, &[f64::NAN, f64::NAN, 1.0]);
        let latest = latest_correlation_coefficient_store(&store, 3).unwrap();
