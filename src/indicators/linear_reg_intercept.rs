...
mod tests {
    ...
    #[test]
    fn linear_regression_intercept_matches_the_line_intercept() {
        let store = close_store(&[2.0, 4.0, 6.0]);
        let values = linear_reg_intercept_store(&store, 3, &mut HashMap::new());

        assert_series_close(&values, &[f64::NAN, f64::NAN, 2.0]);
        assert_eq!(latest_linear_reg_intercept_store(&store, 3), Some(2.0));
    }
}
