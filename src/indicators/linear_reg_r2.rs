...
mod tests {
    ...
    #[test]
    fn linear_regression_r2_is_one_for_a_perfect_line() {
        let store = close_store(&[2.0, 4.0, 6.0]);
        let values = linear_reg_r2_store(&store, 3, &mut HashMap::new());

        assert_series_close(&values, &[f64::NAN, f64::NAN, 1.0]);
        assert_eq!(latest_linear_reg_r2_store(&store, 3), Some(1.0));
    }
}
