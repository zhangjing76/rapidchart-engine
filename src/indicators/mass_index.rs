...
pub fn mass_index_store(store: &CandleStore, period: usize, nodes: &mut NodeCache) -> RcSeries {
    ...
    let hl: Vec<f64> = store.high.iter().zip(store.low.iter()).map(|(h, l)| h - l).collect();
    ...
    for i in 0..len {
        ...
        if valid.len() == period { out[i] = valid.iter().sum(); }
    }
    let rc = Rc::new(out); nodes.insert(key, Rc::clone(&rc)); rc
}
pub fn mass_index_node(bars: &[Bar], period: usize, nodes: &mut NodeCache) -> Series {
    let key = format!("mass:hl:{period}");
    if let Some(v) = nodes.get(&key) { return (**v).clone(); }
    let len = bars.len();
    let mut out = vec![f64::NAN; len];
    if len < 2 || period == 0 { nodes.insert(key, Rc::new(out.clone())); return out; }
