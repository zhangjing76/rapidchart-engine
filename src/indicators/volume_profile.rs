...
pub fn volume_profile(bars: &[Bar], period: usize, _nodes: &mut NodeCache) -> Vec<IndicatorOutput> {
    let len = bars.len();
    ...
    for i in period - 1..len {
        ...
        for j in window_start..=i {
            high = high.max(bars[j].high);
            low = low.min(bars[j].low);
        }
        let range = high - low;
        if range < 1e-10 {
        ...
        for j in window_start..=i {
            ...
            bins[bin] += bars[j].volume;
            total_vol += bars[j].volume;
        }
